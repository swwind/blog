...
<Metadata author="swwind" time="2024-4-16" updated="2024-4-23" />
...
新建一个虚拟网卡，直接桥接到上层的路由器，然后给容器用。

这样容器就可以像主机一样直接向路由器申请和使用 IPv6 地址了，不过要实现数据包过滤会比较麻烦。

但是，我们校园网是根据 MAC 地址认识用户的，所以如果桥接过去路由器就会当作是两台设备，还要重新认证一遍校园网，所以我直接放弃这个方案。
