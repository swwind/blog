...
# 基于 systemd-nspawn 的轻量化容器搭建网络配置
...
1. 本机获取的是一整段 `/64` 地址，即上层路由器会将整段地址的数据包都路由到本机；
2. 本机获取到的只有一个 `/128` 地址，即路由器只会将目标地址完全匹配的数据包路由到本机。

第一种情况比较常用在家庭宽带的路由器中，第二种情况比较常用在校园网和其他环境中。
...
我没有折腾过，你可能需要自己查阅文档。当然，如果你不想给容器分配公网地址，你也可以用下面的方式。

#### 只能用一个地址

针对第二种情况，有很多解决办法，这里给出一种基于 NAT6 的地址分配方式。
...
最后启动 `dnsmasq` 的时候加入以下参数来自动分配地址。

```
--dhcp-range=fd23::/64,ra-stateless,ra-names
```

之后使用 `machinectl` 重启容器，就可以看到容器被自动分配了 `fd23::/64` 的地址，并且可以通过网络地址转换功能接入 IPv6 网络。
